...
int main() {
    ...
    vector<int> idk{1,3,4,5,2,7,8,9,6};
    TreapNode* treap =nullptr;
    treap = new TreapNode(1);
    TreapNode* dusra = new TreapNode(idk[1]);
    treap = merge(treap,new TreapNode(idk[1]));
    for(int i=2;i<idk.size();i++){
        treap = merge(treap,new TreapNode(idk[i]));
    }
    
	// array<Treap*,2> a = split(treap,1);
	// array<Treap*,2> b = split(a[1],3);
	// array<Treap*,2> c = split(b[1],1);
	//treap = merge(a[0],merge(c[1],merge(c[0],b[0])));
	//treap = reverse(treap,2,6);
	//treap = erase(treap,2);
	printInorder(treap);
	cout<<endl<<minimum(treap,0,2);
	deleteTreap(treap);
	return 0;
}