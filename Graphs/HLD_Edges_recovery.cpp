...
class HLD{
    struct Edge{
        int to, index;
        Edge(int _to=0, int _index=-1):to(_to), index(_index){}
    }

    using graph = std::vector<std::vector<Edge>>;
    static constexpr bool vals_in_edges = true;
    private:
    int N;
    graph g;
    ...
    public:
        HLD(const graph &G,int root,vector<long long int> &Uvertex_vals,long long int _id_node):N(G.size()),tree(N,_id_node){
            id_node = _id_node;
            g=G;
            par.assign(N,-1);
            start.resize(N);
